...
create table if not exists public.service_accounts (
  ...
  subscription_email text not null default '',
...
begin
  ...
  if not exists (
    select 1 from pg_constraint where conname = 'service_accounts_service_check'
  ) then
    alter table public.service_accounts
      add constraint service_accounts_service_check
      check (service in ('Grok', 'ChatGPT', 'Perplexity', 'Gemini'));
  end if;
end
$$;
...
drop policy if exists "service_accounts_select_policy" on public.service_accounts;
drop policy if exists "service_accounts_insert_policy" on public.service_accounts;
drop policy if exists "service_accounts_update_policy" on public.service_accounts;
drop policy if exists "service_accounts_delete_policy" on public.service_accounts;

create policy "service_accounts_select_policy"
...
create policy "service_accounts_insert_policy"
on public.service_accounts
for insert
to authenticated
with check (
...
create policy "service_accounts_update_policy"
on public.service_accounts
for update
to authenticated
using (
...
create policy "service_accounts_delete_policy"
on public.service_accounts
for delete
to authenticated
using (
